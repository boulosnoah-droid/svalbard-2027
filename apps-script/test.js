...
let last; for (let i = 0; i < 16; i++) last = post({ kind: "contact", nom: "flood", email: "flood@x.ch", message: "m" });
assert.strictEqual(sheets.Messages.rows.filter((r) => r[2] === "flood").length, 15, "limite 15/h");
assert.strictEqual(last.erreur, "limite", "refus explicite");
